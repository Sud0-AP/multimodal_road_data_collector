...
class RecordingLifecycleNotifier extends StateNotifier<bool>
  ...
  Future<void> _stopRecording() async {
    try {
      ...
      Logger.error('RECORDING', 'Error stopping recording session', e);
    } finally {
      _isRecording = false;
      state = false;
    }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused ||
      ...
      if (_isRecording) {
        Logger.recording(
          'App backgrounded during recording - stopping recording gracefully',
        );
        _stopRecording();
      }
    }
...
final recordingLifecycleProvider =
    StateNotifierProvider<RecordingLifecycleNotifier, bool>((ref) {
